...
class GraphAlgo(GraphAlgoInterface):
    ...
    def save_to_json(self, file_name: str) -> bool:
        ...
        """
        data=ast.literal_eval(self.graph.__str__())
    ...
    def shortest_path(self, id1: int, id2: int) -> (float, list):
        ...
        q = PriorityQueue()
        for n in self.v:
            q.put((self.v[n].GetD(),self.v[n].getKey()))
        while not q.empty():
            u=q.get()[1]
            for y in self.graph.outE[u].keys():
                self.relax(self.v[u],self.v[y],self.graph.outE[u][y])
        list=[]
